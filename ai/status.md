...
- ✅ Rounded map-frame corners clip the map content in the PDF for BOTH export modes: the vector and raster map bodies (and the rotated grid/insets overlay) are wrapped in a `clipToMapBody` rounded-bottom path (`c` corner arcs + `W n`), while the frame border/background keep `roundedRect` on all four corners — verified pixel-wise in both the vector and raster renders (rounded frame cuts the corner, square frame keeps the border through the corner, no tile content pokes past the radius)
